...
export const offerService = {
  ...
  async create(offerData) {
    ...
    // Normalize numeric fields that might be '' to null to avoid 22P02
    ;['base_salary', 'hourly_rate', 'monthly_hours', 'vacation_days'].forEach((key) => {
      if (payload[key] === '') payload[key] = null
    })
    // Normalize date fields that might be '' to null to avoid 22007
    ;['start_date', 'offer_deadline'].forEach((key) => {
      if (payload[key] === '') payload[key] = null
    })
  ...
  async update(id, updateData) {
    ...
    // Normalize empty UUID-like fields to null
    Object.keys(payload).forEach((key) => {
      if (key.endsWith('_id') && payload[key] === '') {
        payload[key] = null
      }
    })
    // Normalize numeric fields that might be '' to null
    ;['base_salary', 'hourly_rate', 'monthly_hours', 'vacation_days'].forEach((key) => {
      if (payload[key] === '') payload[key] = null
    })
    // Normalize date fields that might be '' to null
    ;['start_date', 'offer_deadline'].forEach((key) => {
      if (payload[key] === '') payload[key] = null
    })
